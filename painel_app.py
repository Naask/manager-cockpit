...
# --- ROTAS DE PLANEAMENTO DE PRODUÇÃO (NOVAS) ---
@app.route('/planning')
def planning_page():
...
def get_daily_orders():
    ...
    # ATENÇÃO: A query abaixo simula os dados de status e peso, pois as colunas
    # 'is_washed', 'wash_scheduled_at', 'weight_kg', etc., ainda não existem na base de dados.
    # A junção com 'order_items' é necessária para contar os itens e simular o peso.
    query = f"""
        SELECT
            o.order_id, c.name as customer_name, o.pickup_datetime, o.total_amount,
            DATE(o.pickup_datetime) as delivery_date,
            0 as is_washed,
            0 as is_passed,
            0 as is_packed,
            '' as wash_scheduled_at,
            '' as pass_scheduled_at,
            SUM(oi.quantity * 0.5) as wash_kg, -- Simula 0.5kg por item
            0 as pass_kg
        FROM orders o
        JOIN customers c ON o.customer_id = c.customer_id
        JOIN order_items oi ON o.order_id = oi.order_id
        {where_clause}
        GROUP BY o.order_id, c.name, o.pickup_datetime, o.total_amount
...
def schedule_order_task():
    data = request.get_json()
    order_id = data.get('order_id')
    task_type = data.get('task_type') # 'wash' ou 'pass'
    schedule_date = data.get('schedule_date')

    if not all([order_id, task_type, schedule_date]):
        return jsonify({'error': 'Parâmetros inválidos'}), 400

    print(f"[MODO LEITURA] Pedido {order_id} agendado para '{task_type}' em {schedule_date}.")
    return jsonify({
        'success': True, 
        'message': 'Agendamento simulado com sucesso.'
    }), 200

@app.route('/api/order/cancel-schedule', methods=['POST'])
def cancel_order_schedule():
    """Nova rota para cancelar um agendamento."""
    data = request.get_json()
    order_id = data.get('order_id')
    task_type = data.get('task_type')

    if not all([order_id, task_type]):
        return jsonify({'error': 'Parâmetros inválidos'}), 400

    print(f"[MODO LEITURA] Cancelamento de agendamento para pedido {order_id}, tarefa '{task_type}'.")
    return jsonify({
        'success': True, 
